...
if($_SERVER['REQUEST_METHOD']=="GET" && isset($_REQUEST['userid']) && !empty($_REQUEST['userid'])){
			...
			if($row==0){
				...
				$row=mysql_num_rows($res);
					if($row==0){
		$sql="Select createdlook.look_id,look_description,look_image,lookprice,look_name from createdlook where lower(gender)=(select gender from userdetails where user_id='$userid') order by rand() limit 5 ";
				$res=mysql_query($sql);
		$row=mysql_num_rows($res);
	}
			}
