...
if($_SERVER['REQUEST_METHOD']=="POST" &&  !empty($_POST['userid']) && !empty($_POST['lookid']) && !empty($_POST['lookcomment'])){
	...
	$sql="INSERT INTO userscomment(user_id,look_id,look_comment) Values('$userid','$lookid','$lookcomment')";
	$res=mysql_query($sql);
	$query="SELECT look_comment from createdlook where look_id='$lookid'";
	$res=mysql_query($query);
	$data=mysql_fetch_array($res);
	$totallookcomment=$data['look_comment'] +1;
	$update="Update createdlook SET look_comment='$totallookcomment' where look_id='$lookid'";
