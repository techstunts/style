...
if($_SERVER['REQUEST_METHOD']=="POST" && isset($_REQUEST['email']) && !empty($_REQUEST['email']) && !empty($_REQUEST['password']) && isset($_REQUEST['password']) && !empty($_REQUEST['gender']) && isset($_REQUEST['gender'])){
  ...
  if(isset($_REQUEST['stylishcode']) && !empty($_REQUEST['stylishcode'])){
    ...
    if($gender=='male'){
      $sql="SELECT stylish_id from stylish_info where stylish_gender='female' ORDER BY stylish_id  LIMIT 1";
      $res=mysql_query($sql);
      ...
      $sql="select stylish_id from stylish_info where stylish_id = (select min(stylish_id) from stylish_info where stylish_gender='female' AND stylish_id > $lastuserid)";
      $res=mysql_query($sql);
      $data=mysql_fetch_array($res);
    ...
    $sql="SELECT stylish_id from stylish_info ORDER BY stylish_id DESC LIMIT 1";
    $res=mysql_query($sql);
    $data=mysql_fetch_array($res);
  ...
  if($valid){
    ...
    $pricerange=$clubprice+$ethicprice+$denimprice+$footwearprice;
    if($gender=='male'){
     $sql="SELECT stylish_id from stylish_info where stylish_gender='female' ORDER BY stylish_id DESC LIMIT 1";
      $res=mysql_query($sql);
      $data=mysql_fetch_array($res);
      $lastfemaleid=$data['stylish_id'];
      $sql="SELECT stylish_id from stylish_info where stylish_gender='female' ORDER BY stylish_id ASC LIMIT 1";
      $res=mysql_query($sql);
      $data=mysql_fetch_array($res);
      $firstfemaleid=$data['stylish_id'];
      $sql="SELECT stylish_id from userdetails where gender='male' ORDER BY user_id DESC LIMIT 1";
