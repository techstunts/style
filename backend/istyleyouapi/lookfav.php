...
if($_SERVER['REQUEST_METHOD']=="POST" &&  !empty($_POST['userid']) && !empty($_POST['lookid'])){
	$userid=$_POST['userid'];
	$lookid=$_POST['lookid'];
	$query="SELECT * from createdlook where look_id='$lookid'";
	...
	if($row==1){
			...
			$res=mysql_query($sql);
			$query="SELECT look_fav from createdlook where look_id='$lookid'";
			$res=mysql_query($query);
			$data=mysql_fetch_array($res);
			$lookfav=$data['look_fav'] + 1;
			$update="Update createdlook SET look_fav='$lookfav' where look_id='$lookid'";
