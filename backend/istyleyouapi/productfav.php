...
if($_SERVER['REQUEST_METHOD']=="POST" &&  !empty($_POST['userid']) && !empty($_POST['productid'])){
	...
	$query="SELECT * from lookdescrip where id='$productid'";
	...
	if($row==1){
			...
			$res=mysql_query($sql);
			$query="SELECT product_fav from lookdescrip where id='$productid'";
			$res=mysql_query($query);
			$data=mysql_fetch_array($res);
			$productfav=$data['product_fav'] + 1;
			$update="Update lookdescrip SET product_fav='$productfav' where id='$productid'";
