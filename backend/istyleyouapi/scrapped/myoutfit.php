...
if($_SERVER['REQUEST_METHOD']=="GET" && isset($_REQUEST['userid']) && !empty($_REQUEST['userid'])){
			...
			$sql="Select distinct looks.id as look_id,looks.description, looks.image, looks.price, occasion, looks.name
			...
			while($data=mysql_fetch_array($res)){
				...
				$data= array('lookdetails'=>array('fav'=>'No','lookid'=>$ids[$i][0],'lookdescription'=>$ids[$i][1],'lookimage'=>$ids[$i][2],'lookprice'=>$ids[$i][3],'occasion'=>$ids[$i][4],'lookname'=>$ids[$i][5],'productdetails' => $productarray));
				$abc[]=$data;
				//$total[]=$abc;
				unset($list);
			...
			//fav
				...
				$sql="Select looks.id as look_id,looks.description, looks.image, looks.price, occasion, looks.name
					  from looks
					  where looks.id NOT IN (Select look_id from users_unlike where user_id='$userid')
					  AND looks.id IN (Select look_id from usersfav
			...
			while($data=mysql_fetch_array($res)){
				...
				$data= array('lookdetails'=>array('fav'=>'yes','lookid'=>$ids[$i][0],'lookdescription'=>$ids[$i][1],'lookimage'=>$ids[$i][2],'lookprice'=>$ids[$i][3],'occasion'=>$ids[$i][4],'lookname'=>$ids[$i][5],'productdetails' => $productarray));
				$abc[]=$data;
				//$total[]=$abc;
				unset($list);
