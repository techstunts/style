...
if($_SERVER['REQUEST_METHOD']=="GET" && isset($_REQUEST['userid']) && !empty($_REQUEST['userid'])){
			...
			$sql="Select distinct looks.id as look_id, looks.description, looks.image, looks.price, occasion, looks.name from sendlook join looks on sendlook.look_id=looks.id where sendlook.user_id='$userid' ORDER BY sendlook.send_id DESC";
			$res=mysql_query($sql);
